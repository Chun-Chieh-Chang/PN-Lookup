      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="px-6 py-4">
          ...
          {alternates && alternates.length > 0 && (
            <p className="text-[0.8125rem] text-slate-400 mb-3">
              替代品號：{alternates.join('、')}（綁定任一者皆有效）
            </p>
          )}
          {boundName && (
            <p className="text-[0.8125rem] text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-md px-2 py-1 mb-3">
            ...
            {filteredNames.map((name) => {
              ...
              return (
                <div key={name} className="flex items-center justify-between px-3 py-2 hover:bg-slate-50">
                  ...
                  {isBound ? (
                    <span className="inline-flex items-center gap-1 text-emerald-600 text-[0.8125rem] font-medium shrink-0">
                      <Check className="w-3.5 h-3.5" /> 已綁定
                    </span>
                  ) : (
                    <button
                      onClick={() => { onBind(partNo, name); onClose(); }}
                      className="px-2.5 py-1 text-[0.8125rem] bg-sky-600 hover:bg-sky-500 text-white rounded-md shrink-0 cursor-pointer"
                    >
                      綁定
                    </button>
                  )}
