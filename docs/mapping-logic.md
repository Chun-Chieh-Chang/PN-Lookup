...
> v7.8.7 管線反轉：圖檔品號提取為第一事實來源（孤兒圖歸零），種子檔（產品一覽表.xlsm 轉譯）負責補欄位（品名/客戶/材料）與 BOM 階層基底，兩者經 `mergeDrawingsIntoMaster` 合併為唯一真源。v7.9.1 起加入語意 BOM（`data/semantic-extract.json`，1492 張全量解析）+ ICU 原料料號對照表（`data/icu-parts.json`，167 筆 8 種客戶）導入，v7.9.2 實作五分類體系（原料/物料/零件/組件/SET）。目前 master：**1027 品號 / 209 組 BOM / 563 連結**（詳細品質數據見第 9 節）。
