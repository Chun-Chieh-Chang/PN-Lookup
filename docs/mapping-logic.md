...
| Excel BOM vs 圖檔 BOM 交叉驗證（v7.8.8） | 可比較組件 117（Excel 181 組中有組件圖者）：**一致 45 / 不一致 72**（SB/SC/SD 系列差異主要為：圖檔 BOM 展開至最終零件、Excel 列子組件粒度；圖檔提取漏件如 B-077 待修）；Excel 有但無圖 64（SA0001 等）；圖檔 BOM 有但 Excel 無 127（非 SA/SB/SC/SD 系列，圖檔優先新增） |
| 客戶資訊（v7.8.8 補齊） | 品號掛客戶 **564 / 1004**（Excel 三表有客戶者零漏掛）；**模具號碼 419** / **穴數 414**（internalParts + customerPartNumbers 來源）；剩餘 440 無客戶為圖檔優先收錄品號，Excel 無此資料（含別稱比對 0 可補） |
