...
> v7.8.7 管線反轉：圖檔品號提取為第一事實來源（孤兒圖歸零），種子檔（產品一覽表.xlsm 轉譯）負責補欄位（品名/客戶/材料）與 BOM 階層基底，兩者經 `mergeDrawingsIntoMaster` 合併為唯一真源。目前 master：**961 品號 / 192 組 BOM / 451 連結**（詳細品質數據見第 9 節）。
...
| 196 品號無圖檔可對應 | 零件 136、SA組立 44、SB組立 14、SC組立 1、其他組件 1（如 SA0001、8003875、A01-210-111 等；2026-08-18 重算：master 961 對比圖檔 872 唯一品號） | ⚠️ 資料事實：僅存在於 Excel 種子，無實體圖檔 |
...
| 指標 | 數值 |
|---|---|
| master 品號總數 | **961**（種子 667 + 圖檔提取 292；種子 667 = 693 + scannedAssemblies 24 − 8 筆 MDXE 尾綴版次合併 − 2 筆收縮膜尺寸雜訊 − **40 筆互為別名雙實體合併**；v7.8.14 SPC 圖號註冊格式修正 −1：SPC0005450 圖號實體改為 RAW0000336） |
...
| **SPC 圖號註冊格式（v7.8.14）** | `<圖號>_<版次>_<品號>`（SPC 規格編號 + 原料圖號）整串檢測 `^SPC\d+_\d{1,3}_([A-Z0-9][A-Z0-9_.\-]*)$` → 尾段為品號；圖號（SPC\d+）非品號，組件圖內文出現自身圖號不收入候選；既有單品零件有組件圖 → 升級組件圖候補（64 個）。SPC0005450 → **RAW0000336**（children [E13-999-421, RAW0000335]）；E13-999-421 可組裝目標 13（含 RAW0000336）；master **961 / 192 / 451** |
| **物料類別三層體系（v7.8.15）** | category 統一映射：單品零件 + 零件圖 → **零件**（635）；物料圖 → **物料**（135）；組件圖候補 → **其他組件**（70）；SA/SB/SC/SD 組立 不變（66/34/15/6）；篩選下拉依 物料 / 零件 / 組件 三組 optgroup 分組 |
...
| 客戶資訊（v7.8.8 補齊） | 品號掛客戶 **524 / 961**（Excel 三表有客戶者零漏掛）；**模具號碼 379** / **穴數 376**；剩餘 437 無客戶為圖檔優先收錄品號，Excel 無此資料 |
