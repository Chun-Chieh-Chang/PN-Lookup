...
| `internalParts` | 248 | 產品編號、產品編號(舊)、零件編號(客)、零件名稱、顏色、原料 | 廠內品號（舊編號/客戶編號成為**別稱**） |
| `customerPartNumbers` | 172 | 產品編號(廠內)、零件編號(客)、圖面編號、零件名稱 | **對照表**：廠內品號 ⇄ 客戶料號 ⇄ 圖面編號（圖面編號常出現在圖檔檔名） |
...
- **去重**：以 `partNo` 為 key 存入 `Map`（後筆補齊前筆缺漏欄位：customer / name / color / material / alternates）。
