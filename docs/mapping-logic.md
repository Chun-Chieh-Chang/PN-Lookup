...
> v7.8.7 管線反轉：圖檔品號提取為第一事實來源（孤兒圖歸零），種子檔（產品一覽表.xlsm 轉譯）負責補欄位（品名/客戶/材料）與 BOM 階層基底，兩者經 `mergeDrawingsIntoMaster` 合併為唯一真源。目前 master：**963 品號 / 192 組 BOM / 451 連結**（詳細品質數據見第 9 節）。
...
| BOM 父子連結總數 | **451**（v7.8.11 移除規格書（Supplier Acceptance Letter）與零件圖行尾品號誤建的雜訊連結；v7.8.14 移除 SPC 圖號偽連結） |
...
| **SPC 圖號註冊格式（v7.8.14）** | `<圖號>_<版次>_<品號>`（SPC 規格編號 + 原料圖號）整串檢測 `^SPC\d+_\d{1,3}_([A-Z0-9][A-Z0-9_.\-]*)$` → 尾段為品號；圖號（SPC\d+）非品號，組件圖內文出現自身圖號不收入候選；既有單品零件有組件圖 → 升級組件圖候補（64 個）。SPC0005450 → **RAW0000336**（children [E13-999-421, RAW0000335]）；E13-999-421 可組裝目標 13（含 RAW0000336）；master **963 / 192 / 451** |
| **物料類別三層體系（v7.8.15/v7.8.19）** | category 統一映射：單品零件 + 零件圖 → **零件**（634）；物料圖 → **物料**（137，含 v7.8.19 收錄收縮膜 2）；組件圖候補 → **其他組件**（11，v7.8.18 候補降級後）；SA/SB/SC/SD 組立（95/52/25/9）；篩選下拉依 物料 / 零件 / 組件 三組 optgroup 分組 |
...
| 圖檔提取數 | 1514（組件 249 / 零件 1126 / 物料 139；唯一檔名品號 872，master 命中 100%） |
