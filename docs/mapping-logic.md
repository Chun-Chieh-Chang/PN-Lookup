...
> v7.8.7 管線反轉：圖檔品號提取為第一事實來源（孤兒圖歸零），種子檔（產品一覽表.xlsm 轉譯）負責補欄位（品名/客戶/材料）與 BOM 階層基底，兩者經 `mergeDrawingsIntoMaster` 合併為唯一真源。目前 master：**963 品號 / 209 組 BOM / 563 連結**（詳細品質數據見第 9 節）。
...
| 指標 | 數值 |
|---|---|
| master 品號總數 | **963**（種子 669 + 圖檔提取 292；種子 669 = 693 + scannedAssemblies 24 − 8 筆 MDXE 尾綴版次合併 − **40 筆互為別名雙實體合併** + 2 收縮膜物料 0.08*14mm / 0.08*14.5mm（v7.8.19）；v7.8.14 SPC 圖號註冊格式修正 −1：SPC0005450 圖號實體改為 RAW0000336） |
| BOM 組件數（bom.children 鍵數） | **209**（seed 181 全數保留 + 圖檔組件圖 28；v7.8.20 無表頭 BOM 版式判別 +17：MDXE-*_E×8、BD 組件圖×2、R1-2357/R1-15769/R1-16501/R1-8392/R1-8393/R1-15170；v7.8.11 移除規格書/零件圖誤判的偽組件 65 個） |
| BOM 父子連結總數 | **563**（v7.8.20 +112：無表頭 BOM 版式組件圖 17 組；v7.8.11 移除規格書（Supplier Acceptance Letter）與零件圖行尾品號誤建的雜訊連結；v7.8.14 移除 SPC 圖號偽連結） |
| 有組件（parents）的品號 | 全部可解析為已登錄組件 |
| 無法解析為 master 品號的父鍵 | **0**（v7.8.5 修復前為 51.3%） |
| 孤兒圖（檔名品號未登錄） | **0**（圖檔唯一檔名品號 872 零遺漏；僅 7 張無品號圖：ICU原料料號對照表 + 6 張 XXXX 占位符，正確排除） |
...
| **review 降噪（v7.8.12）** | 標題欄 vs 檔名鍵比對加入①郵遞區號過濾（5 位純數字，如 CA 92673 非品號）、②加工複本別名自洽排除（標題欄廠內編號 R1-15076 等命中 master 別名索引 → 非不一致）；review 46 → **4 條且全已人工確認**（D09-350-211-1 檔名正確、TA161BEPTG012C00/B00 以 Excel 品號為鍵、R1-8391 版次尾綴）；待人工確認事項收斂為 0 |
| **SPC 圖號註冊格式（v7.8.14）** | `<圖號>_<版次>_<品號>`（SPC 規格編號 + 原料圖號）整串檢測 `^SPC\d+_\d{1,3}_([A-Z0-9][A-Z0-9_.\-]*)$` → 尾段為品號；圖號（SPC\d+）非品號，組件圖內文出現自身圖號不收入候選；既有單品零件有組件圖 → 升級組件圖候補（64 個）。SPC0005450 → **RAW0000336**（children [E13-999-421, RAW0000335]）；E13-999-421 可組裝目標 13（含 RAW0000336）；master **963 / 209 / 563** |
| **物料類別三層體系（v7.8.15/v7.8.19/v7.8.20）** | category 統一映射：單品零件 + 零件圖 → **零件**（617）；物料圖 → **物料**（137，含 v7.8.19 收錄收縮膜 2）；組件圖候補 → **其他組件**（28，v7.8.20 無表頭 BOM 版式 +17）；SA/SB/SC/SD 組立（95/52/25/9）；篩選下拉依 物料 / 零件 / 組件 三組 optgroup 分組 |
...
| Excel BOM vs 圖檔 BOM 交叉驗證（v7.8.8） | 一致 47 / 不一致 70；**整合策略（使用者確認）：圖檔為主** — 組件有圖檔 BOM 時以圖檔展開粒度取代 Excel children（SB0001 = B06-410-111-1 + B-077，取代 Excel SA0001 + 0.08*14mm）；SB0055/SB0083 等 7 組逐張核對為粒度差異無衝突（SA0158 = [G13-001-122, E13-999-421] 展開吻合）；版次差異以圖面為準（SA0002 → H00-111-111-4）；收縮膜（0.08*14mm / 0.08*14.5mm）於 v7.8.19 收錄為物料實體（43 組立 children），圖檔 BOM 取代時保留（Gemini 核對 SB0064/SB0065/SB0035/SB0011 證實圖面 KEY UNIT 表有此項） |
| 客戶資訊（v7.8.8 補齊） | 品號掛客戶 **524 / 963**（Excel 三表有客戶者零漏掛）；**模具號碼 379** / **穴數 376**；剩餘 437 無客戶為圖檔優先收錄品號，Excel 無此資料 |
| 無 BOM 參與的品號 | 769（含取代後失去引用的 Excel 概念組件 SA0001 等，資料事實） |
