...
1. 核心數據不變量 (Data Invariants)：
   - 主資料庫 parts 實體必須 100% 保持依 `partNo` 去重，種子轉譯基線嚴格鎖定 669 筆實體（693 種子 + 24 組件圖識別補登 − 8 MDXE 尾綴版次合併 − 40 互為別名雙實體合併 + 2 收縮膜物料 0.08*14mm/0.08*14.5mm（v7.8.19 收錄）；組件圖掃描 --apply 可增量，master ≥ 669）。
