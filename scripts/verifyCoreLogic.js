...
if (existsSync(MASTER_PATH)) {
  ...
  assert(
    parts.length >= 1004,
    `主資料庫品號總數下限固化: 當前 ${parts.length} 筆，必須 >= 圖檔優先管線 1004 筆（種子 709 + 圖檔提取 294 + E09 合併）`
  );
} else {
...
if (existsSync(RAW_SEED_PATH)) {
  ...
  assert(
    convParts.length === 709,
    `種子檔轉譯筆數固化: 預期 709 筆實體品號（693 種子 + 24 組件圖識別補登 scannedAssemblies − 8 筆 MDXE 尾綴版次合併；圖檔提取由 mergeDrawings 增量）`
  );
  assert(
