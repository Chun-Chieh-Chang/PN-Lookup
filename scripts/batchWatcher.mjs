...
async function waitForBatch() {
  ...
  while (true) {
    ...
    if (/===== 完成/.test(content)) return content;
    // 進度快照
    const prog = content.match(/完成 (\d+)\/(\d+)/g);
    if (prog && prog.length && idleLoops % 6 === 0) log(`進度: ${prog[prog.length - 1]}`);
    if (idleLoops > 60 && !/===== 完成/.test(content) && !/node.exe/.test(run('tasklist').toString().slice(0, 0))) {
      // 偵測批次程序是否消失（異常結束）
      try {
        ...
        if (Number(list.trim()) === 0) {
          log('⚠ 批次程序已結束但未見完成標記 — 檢查失敗數後仍執行後續');
          return content;
        }
      } catch { /* ignore */ }
      idleLoops = 0;
...
(async () => {
  ...
  if (existsSync(FLAG)) {
    ...
    const batchOut = await waitForBatch();
    const done = (batchOut.match(/完成：(\d+)\/(\d+)/) || [])[0] || '未知';
    log(`批次完成（${done}）`);
  }
